...
router.get('/me', auth_1.authMiddleware, async (req, res) => {
    const worker = await Worker_1.Worker.findOne({ user_id: req.user.userId }).lean();
    if (!worker) {
...
router.post('/', auth_1.authMiddleware, async (req, res) => {
    const existing = await Worker_1.Worker.findOne({ user_id: req.user.userId });
    if (existing) {
    ...
    await User_1.User.updateOne({ _id: req.user.userId }, { $set: { has_worker: true, active_role: 'worker' } });
...
router.patch('/me', auth_1.authMiddleware, async (req, res) => {
    ...
    const worker = await Worker_1.Worker.findOneAndUpdate({ user_id: req.user.userId }, { $set: updates }, { new: true });
    if (!worker) {
...
router.post('/me/go-live', auth_1.authMiddleware, async (req, res) => {
    ...
    await Worker_1.Worker.updateOne({ _id: worker._id }, {
...
router.post('/me/score-video', auth_1.authMiddleware, async (req, res) => {
    const worker = await Worker_1.Worker.findOne({ user_id: req.user.userId });
    ...
    setTimeout(async () => {
        const scores = (0, utils_1.mockAIScore)();
        await Worker_1.Worker.updateOne({ _id: worker._id }, {
            $set: {
    ...
    await Worker_1.Worker.updateOne({ _id: worker._id }, { $set: { ai_score_status: 'PENDING' } });
...
router.get('/:id/verify', async (req, res) => {
    ...
    try {
        ...
        const worker = await Worker_1.Worker.findById(worker_id).select('full_name profile_photo_url primary_skill trust_score').lean();
